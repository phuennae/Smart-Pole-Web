...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const resetTimer = () => {
      ...
      timeoutRef.current = setTimeout(() => {
        // ✅ ตัด if (logout) ออกเพื่อแก้ Error TypeScript แล้วเรียก Modal แทน alert
  ...
  }, [currentUser]); // ✅ เอา logout ออกจาก dependency (ไม่จำเป็นต้องใส่)
  ...
  return (
    ...
    <div className="flex flex-col md:flex-row h-screen w-screen overflow-hidden bg-gray-100 relative">
      ...
      {/* ✅ Minimal Session Timeout Modal (ธีมสีฟ้า Dev) */}
      {showSessionTimeout && (
          ...
          <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden border border-gray-100 animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100 flex justify-center">
              <button 
                ...
                className="w-full bg-[#48A0D8] text-white py-3 rounded-xl font-bold text-sm hover:bg-blue-600 transition-colors shadow-sm"
              >
                เข้าสู่ระบบใหม่
              </button>
            </div>
